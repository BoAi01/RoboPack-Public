...
The implementation implicitly assumes there is only one object on the table (excluding gripper and the in-hand tool).
"""
...
def shuffle_tensors_based_on_cls(pred_physics, object_cls):
    # Create a list to store shuffled tensors
    shuffled_tensors = []

    # Create a dictionary to group tensors by object_cls values
    tensor_groups = {}
    for i, cls in enumerate(object_cls):
        cls = int(cls.item())
        if cls not in tensor_groups:
            tensor_groups[cls] = []
        tensor_groups[cls].append(pred_physics[i].clone())  # Clone to avoid in-place modification

    # Shuffle the tensors within each group
    for cls, tensors in tensor_groups.items():
        random.shuffle(tensors)

    # Reconstruct the shuffled tensor while retaining object_cls order
    for i in range(len(object_cls)):
        cls = int(object_cls[i].item())
        shuffled_tensors.append(tensor_groups[cls].pop())

    shuffled_pred_physics = torch.stack(shuffled_tensors)

    return shuffled_pred_physics
...
class DynamicsPredictor(pl.LightningModule):
    def __init__(self, config, stats):
        ...
        self.recurr_T = 25  # config["recurr_T"]
        self.obj_phy_feat_len = config["dpi_net"]["obj_phy_feat_len"]
        self.teacher_forcing_thres = config["teacher_forcing_thres"]

        # tactile-related
       # Please note that when a checkpoint is loaded, the config in command line will be over-written by
        # the config in the checkpoint
    ...
    def is_action_valid(self, pointwise_action, min_threashold=1e-4, max_threashold=0.05):
        tool_actions = pointwise_action[:, self.cumul_points[-2]:]
        ...
        for i in range(self.seq_len):
            ...
            if i < updated_recurr_T:
                ...
                if updated_recurr_T > 0:
                    ...
                    pred_physics = shuffle_tensors_based_on_cls(pred_physics, object_cls)
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Calculate total loss over the sequence
        updated_recurr_T = self.recurr_T    # random.randint(0, self.recurr_T - 1)
    ...
    def dynamics_prediction_run_forward(self, action, pos_prev, pred_physics):
        B = action.shape[0]
        N = pos_prev.shape[1]
    ...
    def predict_step(self, observation_batch, action_history, new_action_samples):
        ...
        for t in range(new_action_samples.shape[1]):
            ...
            particle_pos_new = self.dynamics_prediction_run_forward(node_action_feat, particle_pos, pred_physics)
