...
def visualize_points(ax, args, particles, draw_set, target, mask=None, rels=None,
    ...
    if 'floor' in draw_set:
        floor = ax.scatter(particles[args.n_particles:args.n_particles + args.floor_dim, args.axes[0]],
                           ...
                           particles[args.n_particles:args.n_particles + args.floor_dim, args.axes[2]], c='r',
                           s=point_size)
    ...
    if target is not None:
        ax.scatter(target[:, args.axes[0]], target[:, args.axes[1]], target[:, args.axes[2]], c='c', s=point_size,
                   alpha=0.3)
...
def render_anim(args, row_titles, state_seqs, attn_mask_pred=None, rels_pred=None,
                draw_set=['dough', 'floor', 'tool'], axis_off=False, target=None,
    ...
    for frame in frame_list:
        for i in range(n_rows):
            ...
            for j in range(n_cols):
                ax = fig.add_subplot(n_rows, n_cols, i * n_cols + j + 1, projection='3d')
                ax.view_init(*views[j])
                visualize_points(ax, args, state[frame], draw_set, target_cur, axis_off=axis_off, focus=focus_cur,
                                 res=res)
...
def render_o3d(geometry_list, axis_off=False, focus=True, views=[(90, -90), (0, -90), (45, -45)], label_list=[],
               point_size_list=[], path=''):
    n_rows = 2
    n_cols = 3
        ...
        for j in range(n_cols):
            ...
            for k in range(len(geometry_list)):
                ...
                if type == o3d.geometry.Geometry.Type.TriangleMesh:
                    mf = pymeshfix.MeshFix(np.asarray(geometry_list[k].vertices),
                    ...
                    if len(label_list) > 0:
                        label = label_list[k]
                        if 'dough' in label:
                            ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c='b', s=point_size,
                                       label=label)
                        elif 'tool' in label:
                            ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c='r', alpha=0.2, zorder=4.2,
                                       s=point_size, label=label)
                        else:
                            ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c='yellowgreen', zorder=4.1,
                                       s=point_size, label=label)
                    else:
                        label = None
                        ax.scatter(particles[:, 0], particles[:, 1], particles[:, 2], c=colors, s=point_size,
                                   label=label)
                else:
...
def visualize_neighbors(args, particles, target, neighbors, path=''):
    ...
    ax.scatter(particles[neighbors, args.axes[0]], particles[neighbors, args.axes[1]],
               particles[neighbors, args.axes[2]], c='b', s=60)
    ax.scatter(particles[target, args.axes[0]], particles[target, args.axes[1]], particles[target, args.axes[2]], c='r',
               s=60)
...
def visualize_pcd_pred(row_titles, state_list, views=[(90, -90), (0, -90), (45, -45)], axis_off=False, res='low',
                       path=''):
    n_rows = len(row_titles)
    n_cols = len(views)
