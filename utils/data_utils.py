        ...
        for i in range(len(tool_geom_list)):
            ...
            if os.path.exists(tool_repr_points_path):
                ...
                tool_mesh = o3d.io.read_triangle_mesh(
                    os.path.join(args.tool_repr_path, f'{tool_geom_list[i]}_repr.stl'))
    ...
    if args.full_repr:
        ...
        for i in range(fingertip_T_batch.shape[0]):
            if pkg == 'numpy':
                ...
                tool_full_repr = torch.tensor(args.tool_full_repr_dict[args.env][i], dtype=torch.float32,
                                              device=fingertip_T_batch.device)
                tool_repr_part = (fingertip_T_batch[i, :3, :3] @ tool_full_repr.T).T + torch.tile(
                    fingertip_T_batch[i, :3, 3], (tool_dim_list[i], 1))
...
def read_ros_bag(bag_path):
    ...
    for topic, msg, t in bag.read_messages(
            topics=['/cam1/depth/color/points', '/cam2/depth/color/points', '/cam3/depth/color/points',
